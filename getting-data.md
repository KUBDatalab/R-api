...
a wholesome joke of the type said to be told by fathers with a punchline that is often an obvious or predictable pun or play on words and usually judged to be endearingly corny or unfunny. According to [Merriam Webster](https://www.merriam-webster.com/dictionary/dad%20joke){target="_blank"}
:::
...
Taking a quick look at the [documentation](https://icanhazdadjoke.com/api){target="_blank"} we first try GET directly:
...
``` output
[1] "To the person who stole my anti-depressant pills: I hope you're happy now."
```
...
Which translates to a table like this:
...
Response [https://icanhazdadjoke.com/]
  ...
  Size: 149 B
{"id":"VfFlGJmOmb","joke":"People saying 'boo! to their friends has risen by ...
```
...
$joke
[1] "People saying 'boo! to their friends has risen by 85% in the last year.... That's a frightening statistic."
...
We can also search for words in jokes. The [documentation](https://icanhazdadjoke.com/api){target="_blank"} tells us, that we should send our GET request to the URL
...
3                                                                                                                                                                                                                           What did the dog say to the two trees? Bark bark.
...
Request dad jokes about cats using the `GET()` function, and extract the content.

::: solution
## Solution

We've done this earlier, and just have to change "dog" to "cat":


``` r
GET("https://icanhazdadjoke.com/search?term=cat", accept("application/json")) |> 
content(as="text") |> 
fromJSON()
```
